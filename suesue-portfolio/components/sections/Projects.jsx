        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projectData.map((project, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-card rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 border border-border h-full flex flex-col"
            >
              <div className="h-48 bg-muted/40 flex items-center justify-center">
                <span className="text-2xl font-bold text-purple-primary">{project.title.split(" ")[0]}</span>
              </div>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.techs.map((tech, i) => (
                    <Badge key={i} variant="outline" className="bg-secondary/10 border-secondary text-foreground">
                      {tech}
                    </Badge>
                  ))}
                </div>
